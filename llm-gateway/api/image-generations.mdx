...
<Info>
  支持在 `reference_image`、`reference_images`、`image`、`images`、`input_image`、`input_image_url`、`mask` 等字段中传入图片资源。图片资源可以是公网 URL、`data:image/<format>;base64,<data>` 或 `asset://asset_xxx`，详见 [上传资源](/llm-gateway/api/assets-upload)。
</Info>
...
<ParamField body="reference_image" type="string">
  参考图片。直接传资源字符串：公网 URL、data URL 或 `asset://asset_xxx`
</ParamField>

<ParamField body="reference_images" type="array">
  多张参考图片。数组元素支持与 `reference_image` 相同的写法
</ParamField>
