...
description: "查询公开工作流详情、输入参数清单和输出参数清单"
authMethod: "bearer"
---

# 查询工作流详情

根据工作流 ID 查询已发布版本的详情。提交运行前，应先读取 `editable_parameter_manifest`，再按参数清单组装 `inputs`。

<Info>
  **版本说明**: 返回的 `revision_id` 是当前公开发布版本。提交运行时可以传入该值来锁定本次调用的版本。
</Info>
